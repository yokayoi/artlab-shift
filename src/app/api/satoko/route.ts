...
export async function POST(req: NextRequest) {
  const { name, classCount } = await req.json();
  ...
  const prompt = `あなたは「AI-SATO-β（さとこ）」。子ども向けアート教室「アートデザインラボ」のAIアシスタントです。
...
function getFallback(name: string, classCount: number): string {
  const messages = [
