      ...
      {!schedule ? (
        ...
        <>
          ...
          {isPublished && shift && (() => {
            ...
            const tblUids = Object.keys(uidToName);
            if (tblUids.length === 0) return null;
            ...
            shiftSlotKeys.forEach((sk) => { tblDateCounts[sk.date] = (tblDateCounts[sk.date] || 0) + 1; });

            return (
              <div className="mt-6 bg-white rounded-xl border border-gray-200 overflow-hidden max-w-[640px]">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b-2 border-gray-200 bg-gray-50">
                        ...
                        {tblUids.map((uid) => (
                          <th key={uid} className={`text-center px-2 py-2 font-medium text-xs whitespace-nowrap ${uid === user?.uid ? "text-brand-700 bg-brand-50" : "text-gray-700"}`}>
                            {uidToName[uid]}
                          </th>
                        ))}
                      </tr>
                      ...
                      {shiftSlotKeys.map((sk) => {
                        ...
                        return (
                          <tr key={sk.key} className="border-b border-gray-100">
                            ...
                            {tblUids.map((uid) => (
                              <td key={uid} className={`px-2 py-1.5 text-center ${uid === user?.uid ? "bg-brand-50" : ""}`}>
